...
adm_functions = ['Вакансии', 'Черный список', 'Рассылка', 'Провести опрос']
...
def questionnaire(message, num, questions, appForm, start_time=None):
    ...
    if message.text == "Назад ➤":
        ...
        with open("interviewees.json", "r", encoding="UTF-8") as interviewees_base:
            ...
            if num != 0:
                # appForm['answers'].append(message.text)
...
def question_handler(message):
    if message.text == "Назад ➤":
        ...
        sent = bot.send_message(message.chat.id,
                                ...
                                reply_markup=markup)
        bot.register_next_step_handler(sent, vacancy_add)
    else:
